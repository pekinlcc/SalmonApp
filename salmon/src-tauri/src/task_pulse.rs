...
fn render_task_item(t: Task, cls: Classification) -> PulseItem {
    let (priority, why_prefix, label_prefix) = match &cls {
    ...
    // Suggested actions. The task id is encoded in detail strings since
    // ActionStep doesn't carry a structured id field — the briefing
    // executor's existing kinds (acknowledge / reply / calendar) won't
    // touch tasks directly today. Future: add task_toggle / task_delete
    // briefing kinds (see Wave-5 TODO).
    let suggested = vec![
        SuggestedAction {
